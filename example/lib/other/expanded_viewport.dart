...
class _RenderExpandedViewport extends RenderViewport {
  ...
  void performLayout() {
    ...
    while (p != null) {
      totalLayoutExtent += p.geometry?.scrollExtent ?? 0.0;
  ...
  double _attemptLayout(RenderSliver expandPosition, double mainAxisExtent,
    ...
    final double fullCacheExtent = mainAxisExtent + 2 * (cacheExtent ?? 0.0);
    final double centerCacheOffset = centerOffset + (cacheExtent ?? 0.0);
    final double forwardDirectionRemainingCacheExtent =
        (fullCacheExtent - centerCacheOffset).clamp(0.0, fullCacheExtent);

    final RenderSliver? leadingNegativeChild =
        center != null ? childBefore(center!) : null;
    ...
    return layoutChildSequence(
      ...
      cacheOrigin: centerOffset.clamp(-(cacheExtent ?? 0.0), 0.0),
    );
  }
}
